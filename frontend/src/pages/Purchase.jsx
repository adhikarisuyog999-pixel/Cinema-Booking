...
const Purchase = () => {
        ...
        .catch((error) => {
          ...
          setFetchError(
            error?.response?.data?.message ||
              "Unable to load showtime details.",
          );
        })
  ...
  const minsRemaining = showtimeDate
    ? (showtimeDate.getTime() - Date.now()) / (1000 * 60)
    : 999;
  ...
  const onPurchase = async (isPurchased = true) => {
    ...
    try {
      ...
      console.error(error);
      toast.error(
        error?.response?.data?.message || "Unable to complete transaction",
      );
    } finally {
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-xl space-y-4">
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-900 flex items-center gap-3">
              ...
              <span>
                <strong>Requirement:</strong> Reserved tickets must be purchased
                at least 30 minutes before showtime, otherwise the reservation
                expires and becomes available for anyone to buy.
              </span>
            </div>
            ...
            <div className="pt-2 flex flex-col sm:flex-row gap-3">
              ...
              >
                ...
                <span>
                  {isPurchasing ? "Processing..." : "Confirm & Buy Ticket"}
                </span>
              </button>
            </div>
          </div>
