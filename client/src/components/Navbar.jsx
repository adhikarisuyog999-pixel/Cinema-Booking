...
const Navbar = ({ theme = "dark" }) => {
        ...
        <div className="flex grow items-center justify-center gap-3 pt-2 lg:justify-end lg:pt-0">
          {auth.username && (
            <div className="flex items-center gap-2 rounded-full bg-blue-950/80 px-3 py-1 text-xs font-medium text-slate-200 border border-blue-700/50">
              ...
              {auth.role === "admin" && (
                <span className="rounded bg-orange-950 px-1.5 py-0.5 text-[10px] uppercase tracking-wider text-orange-400 font-semibold border border-orange-800">
                  Admin
                </span>
              )}
            </div>
  ...
  return (
    <nav
      className={`sticky top-0 z-50 flex flex-col items-center justify-between px-4 py-3 sm:px-8 lg:flex-row shadow-lg ${
        isLight
          ? "border-b border-slate-200 bg-white text-slate-800"
          : "border-b border-blue-800/40 bg-slate-900/90 backdrop-blur-md text-slate-200"
      }`}
    >
      ...
      <div className="hidden grow justify-between gap-4 lg:flex">
        {menuLists()}
      </div>
      {menuOpen && (
        <div className="mt-3 flex w-full grow flex-col gap-3 rounded-xl bg-slate-900 p-4 border border-blue-800/40 lg:hidden">
          {menuLists()}
        </div>
      )}
    </nav>
